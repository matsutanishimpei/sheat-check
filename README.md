...
### 👨‍🏫 教員画面 (`/`)
* **URL**: ルート（`/`）
* **機能**: 教室レイアウトのドラッグ＆ドロップ作成、ケース管理、Supabase設定の登録、リアルタイム監視、座席ロック、一括リセット。
* **運用**: ここで作成した教室データ（UUID）と、各教員が個別に用意した Supabase 接続情報が D1 データベースに保存されます。また、学生用の QR コードが自動生成されます。

### 👩‍🎓 学生画面 (`/student/:roomId`)
* **URL**: `/student/教室のUUID` （教員画面の QR コードから直接アクセス）
...
> 💡 以前の仕様であった `/?room=UUID` へのアクセスは、後方互換性のため自動的に `/student/UUID` へリダイレクトされます。
