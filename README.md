...
### 👨‍🏫 教員向け画面 (認証保護)
* **ログイン画面 (`/`)**: 
  環境変数 `VITE_TEACHER_PASSWORD` を用いたセキュアなパスワード認証画面。認証成功後に各管理画面にアクセス可能。
* **レイアウトスタジオ (`/room_layout`)**: 
  教室の座席レイアウトを直感的にドラッグ＆ドロップで作成・変更できるエディタ。最大5パターン（Case 1〜5）の座席構成を管理・保存可能。
* **教員用監視モニター (`/seats/monitoring`)**: 
  Supabase Realtime と連携した最大画面の監視コンソール。学生の理解度（OK/NG）のリアルタイム更新、NG席の脈動（パルス）アニメーション、座席ロック、一括リセット、高精度な通信遅延測定をサポート。
...
* **ステータスダッシュボード**:
  チェックインが完了すると、ワンタップで教員に「OK」または「要確認 (NG)」を送信可能。コメントを添えて送信することもできます。
* **自動座席ロック同期**:
